<template>
  <div class="flex justify-center items-center">
    <div class="bg-white">
      <h1 class="text-2xl font-bold text-left mb-14">Add Program</h1>
      <form @submit.prevent="addProgram" class="grid grid-cols-2 gap-4">
...
const addProgram = async () => {
  ...
  const response = await requestStore.postData(
    `${import.meta.env.VITE_API_URL}/admin/general-content/`,
    program,
    tokenStore.token || ""
  );
  if (response) {
